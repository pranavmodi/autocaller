...
for raw in c.list()[1]:
 ...
 try:
  ...
  typ,data=c.uid('SEARCH',None,'SINCE 01-May-2026');ids=data[0].split();coverage.append({'folder':folder,'since':'2026-05-01','messages':len(ids)})
  for start in range(0,len(ids),100):
   ...
   for row in rows or []:
    ...
    if bounce or (matched and folder.strip('"') not in ['Sent','Drafts'] and not any('possiblemind' in x for x in froms)):
     _,parts=c.uid('FETCH',uid,'(BODY.PEEK[])');full=BytesParser(policy=policy.default).parsebytes(next(x[1] for x in parts if isinstance(x,tuple)))
    ...
    for p in matched:
     result[p['email'].lower()].append({'folder':folder,'uid':uid,'from':str(msg.get('From','')),'to':str(msg.get('To','')),'subject':str(msg.get('Subject','')),'date':str(msg.get('Date','')),'message_id':str(msg.get('Message-ID','')),'in_reply_to':str(msg.get('In-Reply-To','')),'references':str(msg.get('References','')),'body_excerpt':text[:2500],'bounce':bounce})
 except Exception as exc:errors.append({'folder':folder,'error':type(exc).__name__+': '+str(exc)})
c.logout();print(json.dumps({'checked_at':datetime.datetime.now(datetime.timezone.utc).isoformat(),'coverage':coverage,'errors':errors,'matches':result}))
