...
import { AlertTriangle, Check, CheckCircle2, Circle, Clock3, ExternalLink, FileStack, Loader2, MailCheck, Plus, RotateCcw, ShieldCheck, Trash2, Sparkles, FileText, Mail } from "lucide-react";
import { apiUrl } from "@/lib/api";
import { JobBrowserApplication } from "@/components/JobBrowserApplication";
...
const phaseNames: Record<string, string> = {
  selecting_resume: "Automatic resume selection",
  queued: "Waiting for the application worker",
  researching: "Company, role and contact research",
  discovering_contacts: "Public web contact discovery",
  fetching_sources: "Source-page verification",
  checking_contacts: "Published and Possible OS contact checks",
  drafting: "Recipient selection and email drafting",
...
function applicationSteps(data: Candidate, ready: boolean): ProgressStep[] {
  ...
  return [
    { title: "Resume selected", detail: ready && data.classification?.resume ? data.classification.resume.filename : "Match the job to a category and select its one-page PDF.", state: state(ready, data.classification?.status === "classifying" || (activePreparation && application.phase === "selecting_resume"), ["selecting_resume"]) },
    { title: "Sources and contact verified", detail: application.recipient ? `${application.recipient.name} · ${application.recipient.email}${application.recipient.evidence.source_type === "possibleos_contact" ? " · Possible OS contact" : ""}` : "Confirm the employer and role, then check published and Possible OS contacts.", state: state(!!application.recipient, activePreparation && ["queued", "researching", "discovering_contacts", "fetching_sources", "checking_contacts", "drafting"].includes(application.phase || "queued"), ["researching", "discovering_contacts", "fetching_sources", "checking_contacts", "drafting", "preparation"]) },
    { title: "Email drafted and audited", detail: application.email ? application.email.subject : "Write the email and check every claim against the resume and sources.", state: state(!!application.email, activePreparation && ["drafting", "auditing"].includes(application.phase || ""), ["drafting", "auditing"]) },
...
export function JobApplicationControls({ job, categories }: { job: Candidate; categories: JobAgentConfig["resume_categories"] }) {
  ...
  const detail = useQuery({ queryKey: ["job-agent", "job", job.id], queryFn: () => jobAgentRequest<Candidate>(`/jobs/${job.id}`), initialData: job, refetchInterval: 5000, refetchIntervalInBackground: true, retry: 1 });
  ...
  const classificationMessage = classification?.reason || (isClassifying
    ...
    : classificationStatus === "pending" && classification?.requested
      ...
      : "The agent will match this job to a category and choose its resume when you start an application.");
  const emptyCategoryLabel = isClassifying ? "Classifying…" : classificationStatus === "pending" ? "Automatic selection when you apply" : "Needs review — choose a category";
  const status = application?.status || "not_started";
  ...
  const summary = statusRefreshFailed
    ? { title: "Status refresh failed", detail: "The worker may have finished, but this window could not retrieve the latest result. Reload the status below.", tone: "amber" }
    : status === "not_started" && suggestedNeedsConfirmation
  ...
  return <div className="space-y-5">
    <section aria-label="Application resume" className="overflow-hidden rounded-2xl border border-violet-200 bg-violet-50/50 shadow-sm">
      <div className="flex flex-wrap items-start justify-between gap-3 p-5">
        <div className="flex min-w-0 basis-full items-start gap-3 sm:flex-1 sm:basis-0"><span className="shrink-0 rounded-xl bg-violet-100 p-2.5 text-violet-700"><FileText className="h-5 w-5" /></span><div className="min-w-0"><p className="text-xs font-semibold uppercase tracking-wide text-violet-700">Your application resume</p><h3 className="mt-1 font-semibold text-neutral-950">{ready ? classification.category_name : isClassifying ? "Choosing the best resume…" : "We’ll select the best match"}</h3><p className="mt-1 max-w-xl break-words text-sm text-neutral-600">{ready ? classification.resume?.filename : "Start a website or email application below. Category matching and resume selection happen automatically."}</p></div></div>
        {ready ? <a className="inline-flex items-center gap-2 rounded-lg border border-violet-200 bg-white px-3 py-2 text-sm font-medium text-violet-800 hover:bg-violet-100" href={pdfUrl(classification.resume!.path)} target="_blank" rel="noopener noreferrer"><ExternalLink className="h-4 w-4" />Preview resume</a> : <span className="inline-flex items-center gap-1.5 rounded-full bg-white px-3 py-1 text-xs font-medium text-violet-700">{isClassifying ? <Loader2 className="h-3.5 w-3.5 animate-spin" /> : <Sparkles className="h-3.5 w-3.5" />}{isClassifying ? "Matching" : "Automatic"}</span>}
      </div>
      <details className="border-t border-violet-200/70 bg-white/70 px-5 py-3" open={classificationStatus === "needs_review"}>
        <summary className="cursor-pointer text-sm font-medium text-violet-800">{classificationStatus === "needs_review" ? "Resume selection needs attention" : "Review or change resume category"}</summary>
        <div className="mt-4 space-y-3">
          <p className="text-sm text-neutral-600">{classificationMessage}</p>
          <label className="block text-sm font-medium">Job category<select aria-label="Job category" className={`${input} mt-2`} disabled={resumeLocked || action.isPending || !data.processing_revision} value={classification?.category_id || ""} onChange={e => action.mutate({ endpoint: "category", body: { category_id: e.target.value || null, revision: data.processing_revision } })}><option value="">{emptyCategoryLabel}</option>{categories.map(category => <option key={category.id} value={category.id}>{category.name}</option>)}</select></label>
          {classification?.category_id && !classification.resume && <p className="text-sm text-amber-800">This category needs a one-page PDF. Assign one in Job Agent Settings or choose another category.</p>}
          {suggestedNeedsConfirmation && !resumeLocked && <button className={button} disabled={action.isPending} onClick={() => action.mutate({ endpoint: "category", body: { category_id: classification?.category_id, revision: data.processing_revision } })}>Use this category</button>}
          {!resumeLocked && <button className="inline-flex items-center gap-1.5 text-xs font-medium text-violet-700 underline" disabled={action.isPending || isClassifying} onClick={() => action.mutate({ endpoint: "classify" })}><RotateCcw className="h-3.5 w-3.5" />{isClassifying ? "Matching…" : "Find the best match now"}</button>}
          {resumeLocked && <p className="text-xs text-neutral-500">The resume choice is locked while this application is in progress.</p>}
        </div>
      </details>
    </section>
    ...
    {!locked && <div className="flex flex-wrap gap-2">{canRetry ? <button className={primary} disabled={action.isPending || !data.processing_revision || job.posting.status === "closed"} onClick={() => send("prepare")}><RotateCcw className={`h-4 w-4 ${action.isPending ? "animate-spin" : ""}`} />{action.isPending ? "Retrying…" : retryLabel}</button> : status === "ready" ? <><button className={button} disabled={action.isPending || !data.processing_revision || job.posting.status === "closed"} onClick={() => send("prepare")}><RotateCcw className="h-4 w-4" />Rebuild draft</button><button className={primary} disabled={action.isPending || !data.processing_revision || job.posting.status === "closed"} onClick={() => send("send")}><MailCheck className="h-4 w-4" />{action.isPending ? "Queuing send…" : "Send via Zoho"}</button></> : status !== "needs_review" && <><button className={button} disabled={action.isPending || !data.processing_revision || job.posting.status === "closed"} onClick={() => send("prepare")}>{action.isPending ? "Starting…" : "Prepare draft"}</button><button className={primary} disabled={action.isPending || !data.processing_revision || job.posting.status === "closed"} onClick={() => send("send")}>{action.isPending ? "Starting…" : "Prepare and send via Zoho"}</button></>}</div>}
