...
type Config = {
  name: string; target_roles: string; preferred_industries: string; industry_mode: Mode;
...
export default function JobSearches({ overview, onOpen }: { overview: Overview; onOpen: (job: Candidate) => void }) {
  ...
  function fresh(): Saved {
    ...
    return { id: "", revision: 0, config: { name: "", target_roles: c.target_roles, preferred_industries: c.preferred_industries,
  ...
  return <section role="tabpanel" id="panel-searches" aria-labelledby="tab-searches" className="space-y-5">
    ...
    {edit && <form className="space-y-5 rounded-2xl border border-sky-200 bg-sky-50/50 p-5" onSubmit={e => { e.preventDefault(); act("save", async () => { await request(`/searches${edit.id ? '/' + edit.id : ''}`, { revision: edit.revision, config: { ...edit.config, employer_urls: edit.config.employer_urls.map(u => u.trim()).filter(Boolean) } }); setEdit(null); refresh(); }); }}>
      ...
      <div className="rounded-xl border border-sky-100 bg-white p-4"><label className={label}>Describe the search<textarea className={input} value={description} onChange={e => setDescription(e.target.value)} placeholder="Find AI agent engineering contracts in legal tech, remote from Colombia, posted this week. Exclude roles requiring a law degree." /></label><button type="button" className={`${button} mt-3`} disabled={!!busy || description.trim().length < 5} onClick={() => act("draft", async () => { const result = await request<{ config: Config }>("/searches/draft", { description }); setEdit(old => old ? { ...old, config: result.config } : old); })}>{busy === "draft" ? <Loader2 className="animate-spin" size={16} /> : <Sparkles size={16} />}Turn into editable settings</button><p className="mt-2 text-xs text-slate-500">Nothing runs until you save and choose Run now, or enable a schedule.</p></div>
      <label className={label}>Search name<input required maxLength={120} className={input} value={edit.config.name} onChange={e => update("name", e.target.value)} placeholder="Legal AI — LATAM remote" /></label>
      <div className="grid gap-4 md:grid-cols-2">
        <label className={label}>Roles and responsibilities · Required<textarea required className={input} value={edit.config.target_roles} onChange={e => update("target_roles", e.target.value)} /></label>
        <div className={label}><label htmlFor="search-industries">Industries</label>{mode("industry_mode")}<textarea id="search-industries" required className={input} value={edit.config.preferred_industries} onChange={e => update("preferred_industries", e.target.value)} /></div>
        <div className={label}><label htmlFor="search-location">Location and remote eligibility</label>{mode("location_mode")}<textarea id="search-location" required className={input} value={edit.config.location_preferences} onChange={e => update("location_preferences", e.target.value)} /></div>
        <div className={label}><label htmlFor="search-employment">Employment type</label>{mode("employment_mode")}<select id="search-employment" className={input} value={edit.config.employment_type} onChange={e => update("employment_type", e.target.value as Config["employment_type"])}><option value="any">Any employment type</option><option value="contract">Contract</option><option value="non_contract">Non-contract</option></select><label className={label}>Posted within (days)<input type="number" min={1} max={365} required className={input} value={edit.config.posted_within_days} onChange={e => update("posted_within_days", Number(e.target.value))} /></label></div>
        <label className={label}>Exclude jobs that…<textarea className={input} placeholder="Require a legal degree; require five years of courtroom experience" value={edit.config.exclusions} onChange={e => update("exclusions", e.target.value)} /></label>
        <label className={label}>Other preferences<textarea className={input} value={edit.config.additional_preferences} onChange={e => update("additional_preferences", e.target.value)} placeholder="Prefer small teams and hands-on ownership" /></label>
      </div>
      <p className="text-xs text-slate-500">Required mismatches are excluded. Missing evidence is shown as uncertain. Preferred mismatches remain visible.</p>
      <label className="flex gap-2 text-sm"><input type="checkbox" checked={edit.config.prefer_overseas_employers} onChange={e => update("prefer_overseas_employers", e.target.checked)} />Prefer employers based outside India</label>
      <details className="rounded-xl border border-slate-200 bg-white p-4"><summary className="cursor-pointer text-sm font-semibold">Sources and search breadth · {edit.config.source_ids.length} boards selected</summary><div className="mt-4 grid gap-2 sm:grid-cols-3">{overview.search_sources.items.map(source => <label key={source.id} className={`flex gap-2 text-sm ${!source.available ? 'text-slate-400' : ''}`}><input type="checkbox" disabled={!source.available} checked={edit.config.source_ids.includes(source.id)} onChange={e => update("source_ids", e.target.checked ? [...edit.config.source_ids, source.id] : edit.config.source_ids.filter(id => id !== source.id))} />{source.name}</label>)}</div><label className={`${label} mt-4`}>Employer careers pages (one URL per line)<textarea className={input} value={edit.config.employer_urls.join('\n')} onChange={e => update("employer_urls", e.target.value.split('\n'))} /></label><div className="mt-4 grid gap-4 sm:grid-cols-2"><label className={label}>Candidate budget per run<input className={input} type="number" min={1} max={100} value={edit.config.max_candidates} onChange={e => update("max_candidates", Number(e.target.value))} /></label><label className={label}>Sources per run<input className={input} type="number" min={1} max={30} value={edit.config.max_sources} onChange={e => update("max_sources", Number(e.target.value))} /></label></div><p className="mt-2 text-xs text-slate-500">Sources rotate across runs. These are visible processing budgets, not a promise to exhaust the internet. Runs have a 30-minute time budget and retain partial results.</p></details>
