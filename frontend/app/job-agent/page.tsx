...
function queueQuery(status: ReviewStatus | "", search: string, page: number, order: string, category: string, source: JobSource | "") {
  const params = new URLSearchParams({ search, page: String(page), order, category });
...
export default function JobAgentPage() {
  ...
  const jobs = useQuery({ queryKey: ["job-agent", "jobs", filter, search, page, order, category, source], queryFn: () => jobAgentRequest<{ items: Candidate[]; total: number; total_pages: number }>(`/jobs?${queueQuery(filter, search, page, order, category, source)}`), refetchInterval: 15000 });
...
function ReviewForm({ job, categories, onSaved }: { job: Candidate; categories: JobAgentConfig["resume_categories"]; onSaved: () => void }) {
  ...
  return <><DialogTitle className="pr-6 leading-snug">{job.posting.title}</DialogTitle><DialogDescription>{job.posting.firm_name} · {job.posting.location || "Location unknown"}</DialogDescription>
    <div className="space-y-4"><div className="flex flex-wrap gap-2 text-xs"><span className="rounded bg-neutral-100 px-2 py-1">Source: {sourceLabel(job)}</span><span className="rounded bg-neutral-100 px-2 py-1">{readable(job.posting.work_arrangement)}</span><span className="rounded bg-neutral-100 px-2 py-1">Source status: {readable(job.posting.status)}</span></div>
