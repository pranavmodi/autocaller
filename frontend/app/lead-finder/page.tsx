...
function LeadDetails({ lead }: { lead: LeadFinderFoundLead }) {
  return (
  ...
  useEffect(() => {
    ...
    async function bootstrap() {
      ...
      try {
        ...
        if (requestedRunId) await loadRun(requestedRunId);
        else if (existingRuns[0]) await loadRun(existingRuns[0].id);
      } catch (cause) {
