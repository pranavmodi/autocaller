...
| `pif aliases-audit` | Report malformed, untrusted, missing, conflicting, and duplicate-canonical firm aliases without writing. |
| `pif aliases-rebuild [--apply]` | Conservatively rebuild domain aliases from canonical domains and operator-verified alternates. Defaults to a dry run; `--apply` performs the transactional rebuild. |
| `pif alias-verify <firm_id> <domain> --evidence-url <url>` | Persist a source-backed alternate domain without replacing the firm's canonical, legacy-ID, or vanity aliases. |
