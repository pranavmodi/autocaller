...
| `pif create --file <profile.json> [--dry-run]` | Create an operator-managed firm. Requires `firm_name` plus `website` or `canonical_website`; rejects duplicate canonical domains and records manual provenance. |
| `pif upsert --file <profile.json> [--dry-run]` | Idempotently create by canonical domain or patch the existing matching firm. Intended for repeated technographic-enrichment imports. |
...
| `contacts select --persona <p> [--vendor filevine] [--fresh/--no-fresh] [--domain-fresh/--contact-fresh] [--direct-email/--allow-role-inbox] [--max-per-firm N] [--max-per-domain N] [--min-staff N] [--max-staff N] [--second-contact-min-team N] [--limit N] [--seed N] [--ids] [--json]` | Select a fresh outreach cohort. Domain freshness, direct named mailboxes, and one contact per domain are defaults. Any prior email log, lead-gen batch item, or outreach send makes the domain non-fresh. Filevine application-domain addresses are always unusable. Optional staff-count filters use the stored PIF roster. `--ids` prints one contact ID per line for `lead-gen add-contacts --from`. |
...
| `lead-gen workshop-link --name <person> --firm <firm> [--title <role> --linkedin-url <url> --json]` | Create or reuse one stable contact-attributed `/w/{code}` workshop URL for a LinkedIn recipient. It generates no message copy and sends nothing. The redirect carries opaque `lc`/`c` values instead of contact PII; inspect clicks in `/click-analytics`. |
| `lead-gen workshop-analytics [--days N] [--limit N] [--events] [--json]` | Show the workshop-only recipient funnel behind `/click-analytics`: raw tracked-link opens, known scanner previews, confirmed visits, prompt reveals, page clicks, scroll depth, and the deduplicated activity feed. |
...
| `lead-gen schedule-wave <batch_id> --subject ... --body-file <tmpl> --transport resend\|zoho_api --start "09:00 PT" [--interval-seconds 90] [--limit N] [--link workshop\|none] [--require-fresh-domain/--allow-touched-domain] [--max-per-domain N] [--max-per-firm N] [--skip-scheduled/--include-scheduled] [--dry-run] [--json]` | Compose from a plaintext template and create approved scheduled actions. The default preflight skips previously touched domains and firms and caps the wave at one address per domain and firm. `--limit` supports staged daily blocks; rerun later with the default `--skip-scheduled` to schedule remaining items. Mints/reuses `/w/` links only with `--link workshop`. |
| `lead-gen wave-rollup <batch_id> [--json]` | Honest wave readout with explicit evidence quality. Page sessions are grouped by `session_id` and reported as `raw_page_sessions` vs `human_page_sessions`: a session counts as human only with interaction evidence — a progressive-funnel gesture (`first_pointer`/`scroll_50`/`content_revealed`/on-page click), or the weaker fallbacks of dwell >10s or first activity >15 min after `email_sent`. Because scanners now execute JS and can emulate dwell, trust `gesture_page_sessions` (any human gesture) and `revealed_page_sessions` (tapped tap-to-reveal) over `human_page_sessions`. Bare `session_ready` beacons land in `suspect_page_sessions`; scanner UAs in `scanner_page_sessions`. |
...
| `lead-finder start [--direction "..."] [--provider openai\|openclaw] [--json]` | Create a durable debug run immediately before step 1. The run snapshots the fixed job, all four baseline context files, and the selected web-research provider in Postgres. |
| `lead-finder provider <run_id> <openai\|openclaw> [--json]` | Persist the provider used by future `web.research_person` calls in one run. It does not change Lead Finder reasoning, which remains on `openclaw/main`, or interrupt an active step. |
...
| `lead-finder restart <run_id> [--direction "..."] [--json]` | Create a new step-0 run linked to the prior run. The prior history remains immutable; the user direction and web-research provider are inherited, while the direction may be overridden and the baseline files are freshly snapshotted. |
| `lead-finder reset-all [--direction "..."] [--provider openai\|openclaw] [--yes] [--json]` | Destructively delete every Lead Finder run, step, gateway attempt, and tool call, then atomically create one fresh step-0 run with the selected web-research provider. Requires interactive confirmation unless `--yes` is supplied. It never deletes other Possible OS data. |
...
bin/possibleos contacts select --persona case_manager --vendor filevine \
  --domain-fresh --max-per-firm 1 --max-per-domain 1 \
  --min-staff 5 --max-staff 25 --seed 42 --limit 15 --ids > /tmp/cohort.txt
...
# rollup note: human_page_sessions needs interaction evidence (>10s dwell,
# on-page click, or arrival >15 min after send); scanner beacons that merely
# run JS show up only in raw_page_sessions / suspect_page_sessions.
```
...
bin/possibleos pif people-options
bin/possibleos pif people --leader leader --role founder_owner --limit 10
...
| method | path | notes |
|--------|------|-------|
| GET  | `/datareturned/script` | Public operator-managed shell script. When disabled, returns a minimal script that posts `{}` to `/datareturned` while preserving the saved script. |
| POST | `/datareturned` | Public JSON object/array receiver; appends payload and bounded request metadata to `data_returned_events`. |
| GET  | `/api/datareturned?limit=100` | Session-gated newest-first operator readout; limit 1–100. Storage is hard-capped at the newest 100 events. |
| POST | `/api/datareturned/prune` | Session-gated immediate retention cleanup; returns deleted and retained counts. |
| GET  | `/api/datareturned/script` | Session-gated saved script and active/no-op state for the operator UI and CLI. |
| PUT  | `/api/datareturned/script` | Session-gated save for the shell script (`{"script":"..."}`, maximum 100,000 characters). |
| PUT  | `/api/datareturned/script/enabled` | Session-gated active/no-op toggle (`{"enabled":true|false}`); preserves saved script text. |
