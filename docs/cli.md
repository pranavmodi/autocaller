...
| method | path | notes |
|--------|------|-------|
| GET  | `/datareturned/script` | Public operator-managed shell script. Serves the built-in diagnostic until an authenticated operator or loopback CLI saves a replacement. |
| POST | `/datareturned` | Public JSON object/array receiver; appends payload and bounded request metadata to `data_returned_events`. |
| GET  | `/api/datareturned?limit=100` | Session-gated newest-first operator readout; limit 1–500. |
