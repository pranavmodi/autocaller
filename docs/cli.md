...
| `lead-finder web-research "<person>" --chunk-id ID [--organization ORG --role ROLE --episode-title TITLE --excerpt TEXT --focus TEXT --json]` | Verify one transcript-supported person through live public-web research and return source-backed current-role, recent-signal, and outreach-angle evidence. It does not add the candidate to results. |
...
| `lead-finder start [--direction "..."] [--json]` | Create a durable debug run immediately before step 1. The run snapshots the fixed job and all four baseline context files in Postgres. |
