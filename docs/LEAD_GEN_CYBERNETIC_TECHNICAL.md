...
| `link_clicked` | tracked `link_events` click attributed to an outreach send, `audit_link_clicks` from `/aiaudit/go` with `raw_event_json.channel = "ai_audit"`, or AI Visibility report clicks from `/v/<code>` with `raw_event_json.channel = "ai_visibility"` | deterministic opened-or-clicked |
