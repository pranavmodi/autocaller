...
class RealtimeVoiceBackend(Protocol):
    ...
    # Inject a system-level nudge into the conversation mid-call and trigger
    # a fresh response. Used to redirect the model (e.g. "hit a DM-personal
    # voicemail — deliver the Case B script now"). Implementations should
    # send the text as a user-role message with a [System: ...] marker so
    # it's interpreted as an out-of-band instruction, then kick a new
    # response turn so the model speaks.
