...
## Review-evidence hook (use review_evidence.primary when present)

The payload may include `review_evidence` with a ranked `items` list and a
chosen `primary` item. When `review_evidence.primary` is present, build the
first-touch hook from it, framing by `primary.kind`:

- **complaint** — empathetic "we fix this." Cite `primary.quote` VERBATIM and
  attributed (`{reviewer_name}` + `{review_date}` when present, else "A recent
  reviewer wrote:"), then pivot to the operational pattern it reveals and how
  the Precise-built system addresses it. Tone: "a queue no one is quite owning,
  not anyone being lazy" — never gotcha.
- **praise** — "scale what you're known for." Cite `primary.quote` verbatim and
  attributed, acknowledge the strength genuinely, then offer to make it durable
  under load (so it never slips on a busy week / as they grow) without adding
  headcount.
- **outcome** — credibility/aspiration. Reference the result (`primary.quote`
  verbatim) lightly, then connect to handling more of that case volume without
  the operational drag.
- **fact** — research-proof personalization. Use `primary.paraphrase` (a fact
  often has no quote — NEVER invent one) to show you looked at this firm
  specifically, then the relevant value prop.

Rules:
- Only `primary.quote` text may appear in quotation marks, reproduced VERBATIM —
  never paraphrase, trim mid-sentence, fix grammar, or fabricate. For a `fact`
  with no quote, state `primary.paraphrase` as your own neutral observation,
  NOT in quotation marks.
- If `review_evidence.primary` is null/absent, do NOT mention reviews at all —
  compose exactly as baseline (firm_behavior / inferred_pain_points).
- Only `review_evidence` content may be cited and nothing in it may be altered.
  Keep every baseline rule (plaintext, no em-dashes, consult signature, no PHI).
  Set requires_human_review when the cited item names a person or a sensitive
  situation.
