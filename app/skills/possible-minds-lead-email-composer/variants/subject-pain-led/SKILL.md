...
that name the operational pain or workflow, NOT Precise Imaging. The Precise
proof still opens the body; the subject's job is recognition of a felt pain.
Keep subjects lowercase-casual, specific, and curiosity-oriented.

Good pain-led subject patterns:

- `every file has a chase`
- `the records chase at {firm short name}`
- `who answers when intake is closed`
- `the tenth status email on the same file`
- `lien follow-up, without the follow-up`

Avoid mentioning Precise Imaging in the subject under this variant; avoid
generic subjects like `Quick question about PI workflow bottlenecks`.
