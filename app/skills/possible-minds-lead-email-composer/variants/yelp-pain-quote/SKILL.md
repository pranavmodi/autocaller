...
## Yelp review hook (use when review_evidence.pain_quote is present)

When the payload's `review_evidence.pain_quote` is non-empty, it is the PRIMARY
hook for the first-touch email:

- After the required Precise-led opener, add a short bridge and cite the quote
  VERBATIM and attributed: e.g. `I went through {firm}'s Yelp recently.
  {review_evidence.reviewer_name} wrote ({review_evidence.review_date}): "…exact
  quote…"`. Quote it exactly as given in `review_evidence.pain_quote` — do not
  paraphrase, trim mid-sentence, or embellish. If reviewer_name / review_date
  are null, attribute as `A recent reviewer wrote:`.
- Then pivot to the operational pain the quote reveals (usually
  client-communication / status-update drift) and how the Precise-built system
  addresses the same pattern, ending with the low-friction diagnostic CTA.
  Mirror the archived precise_pain_4step step-2 tone: empathetic ("a queue no
  one is quite owning, not anyone being lazy"), never gotcha.

When `review_evidence.pain_quote` is null/empty: do NOT mention Yelp, reviews, or
a quote at all — compose exactly as baseline (firm_behavior /
inferred_pain_points). NEVER invent, paraphrase from memory, or imply a review
you do not have.

Safety: the quote is public Yelp text (fine to cite), but it is the ONLY review
content you may use and it must be reproduced unaltered. Keep every baseline
rule (plaintext, no em-dashes, consult signature, no PHI). Set
requires_human_review when the quote names a person or a sensitive situation.
